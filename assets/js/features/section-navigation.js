import { sectionUrl, activeSection } from "../core/section-url.js";

// Share/reload a chapter without creating a history entry for every scroll.
export function initSectionNavigation(mode) {
  const projectIds = [
    "mastadon",
    "photos-cleaner",
    "stock-lab",
  ...
  function target(key) {
    ...
    if (key === "earlier")
      element =
        document.getElementById("earlier") ||
        document.getElementById("earlier-work");
  ...
  function track() {
    ...
    if (
      ...
      new URL(location.href).searchParams.get("section") === "contact"
    )
      key = "contact";
    clearTimeout(settle);
    settle = setTimeout(() => {
      if (!restoring) write(key);
    }, 180);
  }
  function onScroll() {
  ...
  window.addEventListener("popstate", () =>
    restore(new URL(location.href).searchParams.get("section") || "home"),
  );
  function initial() {
