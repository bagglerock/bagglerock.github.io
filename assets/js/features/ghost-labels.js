import { fitLabelSize } from "../core/math.js";
...
export function initGhostLabels(mode) {
    ...
    document.querySelectorAll("[data-ghost]").forEach((element) => {
      ...
      const label =
        style.textTransform === "uppercase"
          ? element.dataset.ghost.toUpperCase()
          : element.dataset.ghost;
      const available =
        element.getBoundingClientRect().width * (project ? 1 : 0.9);
      ...
      const measured =
        ...
        tracking * 100 * Math.max(0, label.length - 1);
      // Small safety margin for platform font rasterization differences.
      const cap = project
        ? Math.min(innerWidth * 0.1, 192)
        : Math.min(innerWidth * 0.19, 240);
      element.style.setProperty(
        "--ghost-fit",
        `${fitLabelSize(available, measured, cap).toFixed(2)}px`,
      );
    });
  }
  function schedule() {
